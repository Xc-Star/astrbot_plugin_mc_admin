...
class ParseConfig:
    ...
    def __init__(
        self, interval: str, head: int, tail: int, name_index: int, strip_quotes: bool
    ):
...
FILE_PARSE_CONFIGS = {
    ".txt": ParseConfig(
        interval="|", head=5, tail=-4, name_index=1, strip_quotes=False
    ),
...
class FileParser:
    ...
    def parse(self, file_path: str, task_id: int) -> tuple[bool, list]:
        """解析文件并返回材料列表"""
        # 获取文件扩展名
        ...
        if file_ext == ".litematic":
            ...
            return self._parse_text_file(
                file_path, task_id, FILE_PARSE_CONFIGS[file_ext]
            )
    ...
    def _load_material_filter_config(self):
        ...
        try:
            ...
            config_path = os.path.join(
                plugin_root, "data", "material_filter_config.json"
            )
            ...
            if os.path.exists(config_path):
                ...
                logger.info(
                    f"材料过滤配置加载成功: 黑名单 {len(self.merge_blacklist)} 项, 映射 {len(self.material_id_mapping)} 项"
                )
            else:
    ...
    def _parse_litematic(self, file_path: str, task_id: int) -> tuple[bool, list]:
        """解析 Litematic 投影文件"""
        try:
            # 解析 litematic 源文件
            ...
            if result is None or "error" in result:
                logger.error(
                    f"错误: {result.get('error', '未知错误') if result else '未知错误'}"
                )
                return False, []
    ...
    def _parse_text_file(
        self, file_path: str, task_id: int, config: ParseConfig
    ) -> tuple[bool, list]:
        """解析文本文件（TXT/CSV）"""
        try:
            ...
            # 提取有效行
            valid_lines = (
                lines[config.head : config.tail]
                if config.tail != -1
                else lines[config.head :]
            )
    ...
    def _parse_line(
        self, line: str, config: ParseConfig, number: int, task_id: int
    ) -> tuple | None:
        """解析单行材料数据"""
        try:
    ...
    def get_stack_size(self, item_name: str) -> int:
        ...
        # 特殊堆叠规则
        if any(
            item_name.endswith(suffix)
            for suffix in ["潜影盒", "旗帜", "盔甲架", "告示牌"]
        ):
