...
class LocUtils:
    ...
    def get_loc_by_name(self, name: str) -> Loc | None:
        ...
        try:
            ...
            if result:
                ...
                return Loc(
                    name=result[1], overworld=result[2], nether=result[3], end=result[4]
                )
            return None
        except Exception as e:
    ...
    def get_all_locations(self) -> list[Loc]:
        ...
        try:
            ...
            for row in results:
                locations.append(
                    Loc(name=row[1], overworld=row[2], nether=row[3], end=row[4])
                )
            return locations
        except Exception as e:
    ...
    def set_loc(self, loc: Loc) -> str:
        ...
        try:
            # 检查位置是否存在
            if not self.get_loc_by_name(loc.name):
