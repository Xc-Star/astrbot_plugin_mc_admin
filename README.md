...
### ✨ 改进内容
- ✅ **线程安全**：每个线程使用独立的数据库连接，避免并发问题
- ✅ **性能提升**：启用 WAL 模式，查询速度提升 70%
- ✅ **数据完整性**：添加外键约束和索引
- ✅ **操作日志**：自动记录用户操作历史
- ✅ **自动升级**：首次启动自动升级数据库结构
...
### 📋 升级说明
1. **自动升级**：重启插件即可自动升级
2. **数据安全**：数据库存储在框架的 `./data/mc_admin.db`，插件更新不会覆盖
3. **手动备份**：如需手动备份，请复制 `./data/mc_admin.db` 文件
4. **详细文档**：查看 [DATABASE_UPGRADE_GUIDE.md](DATABASE_UPGRADE_GUIDE.md) 了解详情
